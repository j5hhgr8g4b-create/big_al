...
Complete on `clean-milestone-4-sync`.

Founder UAT passed for Pantry/Shopping, duplicate import handling, Clear Shopping List, Cook Mode, and the full live Import → Save → Plan → Shop → Cook loop. Second-user Restaurant isolation remained a manual verification gap; later code/RLS review found no demonstrated cross-Restaurant security failure.
