...
export function RecipeCard({ recipe, restaurantId }: { recipe: RecipeCardValue; restaurantId: string }) {
  ...
  return (
    <article className="recipe-card visual-card group relative overflow-hidden transition-transform hover:-translate-y-0.5">
      <Link
      ...
      >
        <div className="recipe-card-image aspect-[16/10] overflow-hidden bg-[var(--color-surface-warm)]">
          {recipe.image_url ? (
            <RecipeImage
        ...
        <div className="recipe-card-copy p-4 pb-3">
          <h3 className="line-clamp-2 text-base font-semibold leading-5 tracking-tight text-[var(--color-purple-900)]">
            {recipe.title}
          </h3>
      ...
      <div className="recipe-card-actions flex min-h-14 items-center justify-end border-t border-[var(--color-border)] px-4 py-2">
